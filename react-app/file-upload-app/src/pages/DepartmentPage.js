...
const RoleBadge = ({ role }) => {
  ...
  return <span style={{ padding: '2px 8px', borderRadius: '10px', fontSize: '11px', fontWeight: '700', backgroundColor: cfg.bg, color: cfg.color }}>{cfg.icon} {cfg.label}</span>;
};
...
const ManagerInput = ({ value, onChange, onSelect, cognitoUsers }) => {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <input type="text" placeholder="Type name or pick ▾" value={value} onChange={e => { onChange(e.target.value); setShow(true); }} onFocus={() => setShow(true)} style={inp} />
          ...
          {filtered.map(u => (
            <div key={u.email} onClick={() => { onSelect(u); setShow(false); }}
              ...
              <div>
                ...
                <div style={{ fontSize: '11px', color: '#888' }}>{u.email} · <RoleBadge role={u.role||'MEMBER'} /></div>
              </div>
            </div>
          ))}
        </div>
  ...
  <div style={{ marginTop: '16px', backgroundColor: '#e8f5e9', border: '2px solid #4caf50', borderRadius: '10px', padding: '16px' }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
      ...
      <button onClick={() => { navigator.clipboard.writeText(password); alert('✅ Copied!'); }}
        style={{ padding: '10px 14px', backgroundColor: '#0066ff', color: 'white', border: 'none', borderRadius: '7px', cursor: 'pointer', fontWeight: '600', fontSize: '13px', whiteSpace: 'nowrap' }}>📋 Copy</button>
    </div>
    <div style={{ fontSize: '12px', color: '#888', marginBottom: '10px' }}>⚠️ Save this password and share it with the user.</div>
...
const DepartmentPage = ({ user }) => {
  ...
  const openMembers = async (dept) => {
    ...
    try {
      ...
      // Auto-sync Cognito users whose dept matches
  ...
  const handleProposeRole = async () => {
    ...
    try {
      ...
      if (res.autoApproved) {
        ...
        // ── Immediately update ALL departments in local state ──────────────
        setDepartments(prev => prev.map(d => {
          const ml = Array.isArray(d.membersList) ? d.membersList : [];

          // 1) Remove this user from every dept that is NOT the target dept
          ...
          if (!isTargetDept) {
            ...
            // Also clear manager if this user was the manager
          ...
          // 2) In the target dept, update the user's role
          const updatedMl = ml.map(m => m.email === roleTarget.email ? { ...m, role: proposedRole } : m);
          // Set as manager if becoming head of this dept/unit type
        ...
        // ── Reload after 3s to sync fully with backend ────────────────────
  ...
  const generatePDFReport = () => {
    ...
    const rows = departments.map(d => {
      ...
      return `<tr><td>${d.name}</td><td>${d.type||'department'}</td><td>${d.manager||'—'}</td><td>${d.members||0}</td><td>${d.projects||0}</td>
        <td><span style="padding:3px 10px;border-radius:12px;background:${d.status==='Active'?'#e7f7ef':'#ffeaea'};color:${d.status==='Active'?'#2e7d32':'#c62828'}">${d.status||'Active'}</span></td>
        <td>${ml.map(m=>`${m.name}(${m.role})`).join(', ')||'—'}</td></tr>`;
    }).join('');
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Cloudly Report</title>
    <style>body{font-family:'Segoe UI',sans-serif;padding:40px}h1{color:#0066ff}
    table{width:100%;border-collapse:collapse;font-size:13px}th{background:#0066ff;color:#fff;padding:10px}
                    ...
                    <tr key={dept.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                      ...
                      <td style={{ padding: '12px 14px', fontSize: '14px' }}>
                        {dept.manager === 'Not assigned' ? (
                          <span style={{ color: '#ef5350', fontSize: '13px', fontStyle: 'italic' }}>⚠️ Not assigned</span>
                        ) : dept.manager}
                      </td>
                      <td style={{ padding: '12px 14px' }}>
              ...
              <div>
                {!loadingUsers && cognitoUsers.length > 0 && (
                  <div style={{ marginBottom: '14px' }}>
                    <input type="text" placeholder="🔍 Search by name…" value={cognitoSearch} onChange={e => setCognitoSearch(e.target.value)} style={inp} />
                  </div>
                )}
                {loadingUsers ? (
                  <div style={{ textAlign: 'center', padding: '30px', color: '#888' }}>⏳ Loading users…</div>
                ) : cognitoUsers.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '30px', color: '#aaa' }}><div style={{ fontSize: '36px' }}>☁️</div><div>No Cognito users found.</div></div>
                ) : (
              ...
              <div>
                {currentMembers.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '30px', color: '#aaa' }}>
                    <div style={{ fontSize: '36px', marginBottom: '8px' }}>👤</div><div>No members yet.</div>
                  </div>
                ) : (
      ...
      {showRoleModal && roleTarget && (
        <div style={backdrop} onClick={() => setShowRoleModal(false)}>
          <div style={modalBox('480px')} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ fontSize: '20px', fontWeight: '700' }}>🔄 Change Role</h2>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <textarea value={roleReason} onChange={e => setRoleReason(e.target.value)} rows={3} style={{ ...inp, resize: 'vertical' }} placeholder="Why this role change?" />
            </div>
            {roleMessage && (
            ...
            {isAdmin ? (
              <div style={{ backgroundColor: '#e8f5e9', borderRadius: '8px', padding: '10px', marginBottom: '16px', fontSize: '13px', color: '#2e7d32' }}>
                ✅ As SUPER_ADMIN, applied <strong>immediately</strong>. Page reloads automatically.
              </div>
            ) : (
              <div style={{ backgroundColor: '#fff3e0', borderRadius: '8px', padding: '10px', marginBottom: '16px', fontSize: '13px', color: '#e65100' }}>
                ⚠️ Needs <strong>SUPER_ADMIN</strong> approval.
              </div>
            )}
            ...
            {!createManagerTempPass ? (
              <>
                <div style={{ backgroundColor: '#e3f2fd', borderRadius: '8px', padding: '12px', marginBottom: '20px', fontSize: '13px', color: '#1565c0' }}>
                  ℹ️ Creating manager for <strong>{pendingDeptData?.name}</strong> — assigned as <strong>{pendingDeptData?.type === 'unit' ? 'UNIT_HEAD' : 'DEPT_HEAD'}</strong>.
                </div>
                {[['First Name *','firstName','text'],['Last Name','lastName','text'],['Email *','email','email']].map(([label,key,type]) => (
                  <div key={key} style={{ marginBottom: '14px' }}>
                    <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: '600', color: '#555' }}>{label}</label>
                    <input type={type} value={newManager[key]} onChange={e => setNewManager(p => ({ ...p, [key]: e.target.value }))} style={inp} placeholder={key === 'firstName' ? 'e.g., Jean' : key === 'lastName' ? 'e.g., Dupont' : 'e.g., jean@company.com'} />
                  </div>
                ))}
                ...
                <div style={{ backgroundColor: '#fff3e0', borderRadius: '8px', padding: '10px', marginBottom: '20px', fontSize: '13px', color: '#e65100' }}>
                  ⚠️ A temporary password will be auto-generated.
                </div>
                <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                  <button onClick={() => setShowCreateManager(false)} style={btn('#888')}>Cancel</button>
                  <button onClick={handleCreateManager} disabled={loading} style={{ ...btn('#0066ff'), opacity: loading ? 0.6 : 1 }}>{loading ? '⏳ Creating…' : '✅ Create & Save'}</button>
                </div>
              </>
            ) : (
